...
func (c *Endpoints) Get(name string) (*api.Endpoints, error) {
	...
	resource := fmt.Sprintf("%s/%s/%s", c.namespace, model.ResourceTypeEndpoint, name)
	endpointMsg := message.BuildMsg(modules.MetaGroup, "", pkg.ModuleNameEdgeMesh, resource, model.QueryOperation, nil)
